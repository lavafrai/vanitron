...
pub fn print_result(wallet: &KeyGenerationResult, tries: u64, elapsed: Duration) {
    ...
    anstream::println!("{label}Address:{label:#} {val}{addr}{val:#}",
        label = label,
        val = val_accent,
        addr = wallet.address
    );
    if !wallet.derivation_path.is_empty() {
        anstream::println!("{label}Derivation path:{label:#} {val}{path}{val:#}",
            label = label,
            val = val_accent,
    ...
    anstream::println!("{label}Mnemonic:{label:#} {val}{mnem}{val:#}",
        label = label,
        val = val_accent,
